...
export const listMyNotifications = asyncHandler(async (req: Request, res: Response) => {
  if (!req.user) throw new AppError(t(req, 'notification.unauthorized', { ns: 'errors' }), 401);
...
export const saveFcmToken = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!token) throw new AppError(t(req, 'notification.tokenRequired', { ns: 'errors' }), 400);
